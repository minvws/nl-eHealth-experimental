...
#define NID_JOINT_RESEARCH_CENTRE_EU 1847
#define NID_EHEALTH 2021
#define NID_ZKP_PUBLIC_KEY 1
#define NID_ZKPL_ALG 2
#define NID_CL_ALG NID_ZKPL_ALG.1
#define NID_BBSPLUS_ALG NID_ZKPL_ALG.2

#define xstr(s) str(s)
#define str(s) #s

#define OID_STR_ZKP "1.3.6.1.4.1." xstr(NID_JOINT_RESEARCH_CENTRE_EU) "." xstr(NID_EHEALTH) "." xstr(NID_ZKP_PUBLIC_KEY)
#define OID_STR_CL "1.3.6.1.4.1." xstr(NID_JOINT_RESEARCH_CENTRE_EU) "." xstr(NID_EHEALTH) "." xstr(NID_CL_ALG)

DEFINE_STACK_OF(BIGNUM)

typedef struct {
    ASN1_OBJECT * algorithm;
    ASN1_INTEGER * counter /*, expiry, epoch */; 
    BIGNUM *n, *Z, *S;
...
ASN1_SEQUENCE(ZKP_PUBLIC_KEY) = {
	...
	// ASN1_SIMPLE(ZKP_PUBLIC_KEY, epoch, ASN1_INTEGER),
	// ASN1_SIMPLE(ZKP_PUBLIC_KEY, expiry, ASN1_INTEGER),
...
static char * get_element(char *path, xmlNode * a_node) {
	return _get_element(path,"",a_node);
}

static ASN1_INTEGER * get_ASN1_INTEGER(char *path, xmlNode * a_node) {
...
static BIGNUM * get_BIGNUM(char *path, xmlNode * a_node) {
    BIGNUM *bn = NULL;
    char *str;
